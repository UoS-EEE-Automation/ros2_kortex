...
void KortexMultiInterfaceHardware::createModel()
{
    KDL::Tree tree;
    urdf::Model model;
    urdf_ = info_.original_xml;
    if (!model.initString(urdf_))
    {
      RCLCPP_INFO(LOGGER, "the model failed");
    }
    else
    {
      RCLCPP_INFO(LOGGER, "the model passed");
    }
    if (!kdl_parser::treeFromUrdfModel(model, tree))
    {
      RCLCPP_INFO(LOGGER, "the tree failed");
    }
    else
    {
    ...
    {
      RCLCPP_INFO(LOGGER, "the link passed");
    }
